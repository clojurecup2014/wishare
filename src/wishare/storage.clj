  ...
  (defn get-timeline-record-by-id [timeline-id]
    ...
    (let [fields [:user :text :timestamp]]
      (zipmap fields
              (first (d/q '[:find ?user ?text ?timestamp
                            ...
                            [?t :timeline/user ?user]
                            [?t :timeline/timestamp ?timestamp]
                            ]
                          (d/db conn)
                          timeline-id)))))

  ;;(get-user-timeline "Jhon")
  ;;(get-timeline-record-by-id 17592186045542)
  ...
  (defn find-user-own-timeline [user & {:keys [limit] :or {limit 10}}]
    "Вернет статус подарка для юзера"
    (let [timeline (get-user-timeline user)]
      (map (partial apply get-wish-by-id) timeline)))


  (defn find-user-timeline [user & {:keys [limit] :or {limit 10}}]
      (->> (find-friends user)
           (map get-user-timeline)
           doall))
